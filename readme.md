...
## Generating Migrations strictly from the CLI ##

This functionality only works when arguments are passed to the command `cake Migrations.migration generate`, and adds the ability to perform commands such as the following:
...
Field types are specific to the database connection in use specified by the `--connection` argument (`default` by default). Thus, valid `Postgres` field types include `inet` and `number`, while for MySQL the fieldTypes would be ignored.
