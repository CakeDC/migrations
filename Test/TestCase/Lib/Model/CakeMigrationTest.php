 ...
 */
namespace Migrations\Test\TestCase\Lib\Model;
 ...
 */
class CakeMigrationTest extends CakeMigration {
 ...
 */
	public function testCreateDropTable() {
		...
		$sources = $this->db->schemaCollection()->listTables();
		$this->assertFalse(in_array($this->dataSource->fullTableName('migration_user', false, false), $sources));
		$this->assertFalse(in_array($this->dataSource->fullTableName('migration_posts', false, false), $sources));
		$this->assertTrue($migration->run('up'));
		$sources = $this->db->schemaCollection()->listTables();
		$this->assertTrue(in_array($this->dataSource->fullTableName('migration_users', false, false), $sources));
		$this->assertTrue(in_array($this->dataSource->fullTableName('migration_posts', false, false), $sources));

		try {
			$migration->run('up');
			$this->fail('No exception triggered');
		} catch (MigrationException $e) {
			$this->assertEquals('Table "' . $this->dataSource->fullTableName('migration_posts', false, false) . '" already exists in database.', $e->getMessage());
		}

		$this->assertTrue($migration->run('down'));
		$sources = $this->db->schemaCollection()->listTables();
		$this->assertFalse(in_array($this->dataSource->fullTableName('migration_users', false, false), $sources));
		$this->assertFalse(in_array($this->dataSource->fullTableName('migration_posts', false, false), $sources));
	}
 ...
 */
	public function testRunDownWithPrefix() {
		...
		$sources = $this->db->schemaCollection()->listTables();
		$this->assertFalse(in_array($this->dataSource->fullTableName($prefix . 'migration_user', false, false), $sources));
		$this->assertFalse(in_array($this->dataSource->fullTableName($prefix . 'migration_posts', false, false), $sources));
		$this->assertTrue($migration->run('up'));
		$sources = $this->db->schemaCollection()->listTables();
		$this->assertTrue(in_array($this->dataSource->fullTableName($prefix . 'migration_users', false, false), $sources));
		$this->assertTrue(in_array($this->dataSource->fullTableName($prefix . 'migration_posts', false, false), $sources));

		$this->assertTrue($migration->run('down'));
		$sources = $this->db->schemaCollection()->listTables();
		$this->assertFalse(in_array($this->dataSource->fullTableName($prefix . 'migration_users', false, false), $sources));
		$this->assertFalse(in_array($this->dataSource->fullTableName($prefix . 'migration_posts', false, false), $sources));
	}
 ...
 */
	public function testRenameTable() {
		...
		$sources = $this->db->schemaCollection()->listTables();
		$this->assertTrue(in_array($this->dataSource->fullTableName('posts', false, false), $sources));
		$this->assertFalse(in_array($this->dataSource->fullTableName('renamed_posts', false, false), $sources));

		$this->assertTrue($migration->run('up'));
		$sources = $this->db->schemaCollection()->listTables();
		$this->assertFalse(in_array($this->dataSource->fullTableName('posts', false, false), $sources));
		$this->assertTrue(in_array($this->dataSource->fullTableName('renamed_posts', false, false), $sources));

		try {
			$migration->run('up');
			$this->fail('No exception triggered');
		} catch (MigrationException $e) {
			$this->assertEquals($e->getMessage(), 'Table "' . $this->dataSource->fullTableName('renamed_posts', false, false) . '" already exists in database.');
		}
 ...
 */
	...
	public function testTruncateLongIndexKey() {
		...
		$sources = $this->db->schemaCollection()->listTables();
		$this->assertFalse(in_array($this->dataSource->fullTableName('migration_categories', false, false), $sources));
		try {
			$migration->run('up');
			$this->fail('No exception triggered');
		} catch (MigrationException $e) {
