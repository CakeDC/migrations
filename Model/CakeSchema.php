	...
	public function read($options = array()) {
		...
		if (is_array($models)) {
			foreach ($models as $model) {
				...
				$fulltable = $table = $driver->fullTableName($Object, false, false);
				...
				if (empty($tables[$table])) {
					...
					$tables[$table]['indexes'] = $driver->index($Object);
					$tables[$table]['tableParameters'] = $driver->readTableParameters($fulltable);
				...
				foreach ($Object->hasAndBelongsToMany as $assocData) {
					...
					$withTable = $driver->fullTableName($Object->$class, false, false);
					...
					if (in_array($withTable, $currentTables)) {
						...
						$tables[$noPrefixWith]['indexes'] = $driver->index($Object->$class);
						$tables[$noPrefixWith]['tableParameters'] = $driver->readTableParameters($withTable);
						unset($currentTables[$key]);
					}
				}
			}
		...
		if (!empty($currentTables)) {
			foreach ($currentTables as $table) {
				...
				$fulltable = $driver->fullTableName($Object, false, false);

				if (in_array($table, $systemTables)) {
					$tables[$Object->table] = $this->_columns($Object);
					$tables[$Object->table]['indexes'] = $driver->index($Object);
					$tables[$Object->table]['tableParameters'] = $driver->readTableParameters($fulltable);
				} elseif ($models === false) {
					$tables[$table] = $this->_columns($Object);
					$tables[$table]['indexes'] = $driver->index($Object);
					$tables[$table]['tableParameters'] = $driver->readTableParameters($fulltable);
					...
					$tables['missing'][$table]['indexes'] = $driver->index($Object);
					$tables['missing'][$table]['tableParameters'] = $driver->readTableParameters($fulltable);
				}
			}
		}
 ...
 */
	public function write($object, $options = array()) {
		...
		$out  = "use Migrations\Model\CakeSchema;\n\n";
