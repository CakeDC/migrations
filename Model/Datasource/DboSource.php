 ...
 */
namespace Migrations\Model\Datasource;
...
/**
 * A reference to the driver of this DataSource
 *
 * @var array
 */
 ...
 *
 * @param Connection $db Connection object for this connection.
 */
	public static function get(Connection $db) {
		if (static::$_driver) {
			return static::$_driver;
		}

		if ($driver = get_class($db->config()['datasource'])) {
			$token = explode('\\', $driver);
			$driver = 'Migrations\\Model\\Datasource\\Database\\' . array_pop($token);
			static::$_driver = new $driver($db->config());
		}
